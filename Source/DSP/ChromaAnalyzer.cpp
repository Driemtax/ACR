...
/**
 * @brief Processes every frame of a given spectrogram and calculates the HPCP
 * vector.
 *
 * This method iterates over all frames of the input spectrogram, computes the
 * Harmonic Pitch Class Profile (HPCP) for each frame, normalizes the resulting
 * vectors to the range [0, 1], and applies a median filter to smooth the output
 * chromagram.
 *
 * @param spectogram A constant reference to the input audio buffer containing
 * the spectrogram data.
 * @param outChromagram A reference to the output audio buffer where the
 * computed and filtered chromagram will be stored.
 */
void ChromaAnalyzer::extractChroma(const juce::AudioBuffer<float> &spectogram,
                                   juce::AudioBuffer<float> &outChromagram) {
...
/**
 * @brief Performs a full Harmonic Pitch Class Profile (HPCP) calculation on a
 * single frame of the spectrogram.
 *
 * For detailed information regarding the underlying HPCP algorithm, please
 * refer to docs/DSP.
 *
 * @param currentFrame A pointer to the array containing the magnitude values of
 * the current spectrogram frame.
 * @param frameNum The index of the frame currently being processed.
 * @param outChroma A reference to the output audio buffer where the computed
 * chroma values will be written.
 */
void ChromaAnalyzer::processFrame(const float *currentFrame, int frameNum,
 ...
 *
 * This method iterates over the frequency bins of the provided frame and
 * identifies local maxima by comparing each bin with its immediate left and
 * right neighbors. It only considers peaks within a specific frequency range
 * (approximately 100 Hz to 5000 Hz) and those that exceed a calculated
 * magnitude threshold to ignore noise in quiet frames. The identified peaks are
...
void ChromaAnalyzer::extractPeaks(const float *currentFrame) {
  ...
  int numFrequencies = static_cast<int>(fftBinFrequencies.size());
  float binWidth = fftBinFrequencies[1];
  int startIndex = std::max(2, static_cast<int>(100.f / binWidth));
  int endIndex =
      std::min(numFrequencies - 1, static_cast<int>(5000.0f / binWidth) + 1);
  Peak peak(0.0f, 0.0f);
  ;

  float maxValInFrame =
      *std::max_element(currentFrame + startIndex, currentFrame + endIndex);
  // the thresholds needs to be at a minimum of 0.001 so that a silent frame
  // does not get peaks due to noise
  float threshold = std::max(maxValInFrame * 0.1f, 0.001f);

  // We iterate over all Frequency Bins of this frame and search for a local
  // maxima considering the left and right neighbour frame we start at Index 1,
  // cause index 0 has no left neighbour. We also end one bin early, cause last
  // bin has no right neighbour.
  for (int i = startIndex; i < endIndex; i++) {
    if (currentFrame[i] > currentFrame[i - 1] &&
        currentFrame[i] > currentFrame[i + 1] && currentFrame[i] > threshold) {
 ...
 *
 * This method computes the center frequency for each Harmonic Pitch Class
 * Profile (HPCP) bin based on a reference frequency. The calculated frequencies
 * are spaced logarithmically and stored in the internal binMids array.
 */
void ChromaAnalyzer::calculateBinMids() {
  int numBins = static_cast<int>(binMids.size());
  for (int i = 0; i < numBins; i++) {
    binMids[i] =
        f_ref * powf(2, static_cast<float>(i) / static_cast<float>(numBins));
  }
}
 ...
 *
 * This method computes the distance in semitones between a given frequency and
 * a reference bin frequency. The resulting distance is normalized to be within
 * the range [-6, 6] to represent the shortest path in the circular pitch class
 * space.
 *
 * @param f_i The input frequency to calculate the distance for.
 * @param f_n The reference frequency.
 * @return The shortest distance in semitones between the two frequencies.
 * Returns INFINITY if f_n is less than 0.1.
 */
float ChromaAnalyzer::calculateDistance(float f_i, float f_n) const {
  ...
  float dist = static_cast<float>(fmod(rawDist, 12));
 ...
 *
 * This method computes the weighting factor for a frequency based on its
 * distance to the center frequency of the specified Harmonic Pitch Class
 * Profile (HPCP) bin. It uses a cosine-squared window, ensuring that only
 * frequencies within a certain distance contribute to the bin's energy. For
 * further mathematical details, please refer to docs/DSP.
 *
 * @param n The index of the HPCP bin.
 * @param dist The distance between the frequency and the center frequency of
 * the bin.
 * @return The computed weight for the frequency.
 */
float ChromaAnalyzer::calculateWeightFreq(float dist) const {
  float weight = 0.0;
  float halfPi = juce::MathConstants<float>::halfPi;

  if (std::abs(dist) <= 0.5 * l) {
    float rootedWeight = static_cast<float>(cos(halfPi * (dist / (0.5 * l))));
 ...
 *
 * @param outChroma A reference to the audio buffer containing the chromagram
 * data to be normalized in-place.
 */
void ChromaAnalyzer::normalizeBins(juce::AudioBuffer<float> &outChroma) {
  ...
  for (int i = 0; i < frameCount; i++) {
    ...
    for (int j = 0; j < binCount; j++) {
      ...
      // std::cout << "Frame: " << i << ", Bin:" << j << ", Energy: " <<
      // framePointer[j] << std::endl;
    }
  }
}

/**
 * @brief Applies a median filter across the frames of the chromagram to smooth
 * the data.
 *
 * This method iterates over each chroma bin and applies a moving median filter
 * across consecutive frames. This smoothing process reduces noise and isolated
 * spurious peaks in the chroma features, improving the temporal stability of
 * the output chromagram. The filtering is performed using an internal copy of
 * the buffer to prevent the processed frames from recursively affecting the
 * subsequent frames.
 *
 * @param chroma A reference to the audio buffer containing the chromagram data
 * to be filtered in-place.
 */
void ChromaAnalyzer::applyMedianFilter(juce::AudioBuffer<float> &chroma) {
  // It's just me and the OG Boys. We cant write directly in-place, because then
  // the results of one frame would influence the next frames result.
  ...
  for (int frame = 0; frame < maxFrames; frame++) {
    ...
    for (int bin = 0; bin < chroma.getNumSamples(); bin++) {
      // We iterate over the bin of the left and right neighbors and calculate
      // the median.
