...
public:
  ...
  // classifies a full chromagram frame by frame and returns an array of ints,
  // representing the chord classifications.
  void classifyFullChroma(const juce::AudioBuffer<float> &chroma,
...
private:
  ...
  // chord. Returns -1 if no template is similiar enough to feature vector
  int classifyFrame(const float *frame) const;
  void generateTemplates();
  float calculateCosineSimilarity(const std::vector<float> &templateFeatures,
