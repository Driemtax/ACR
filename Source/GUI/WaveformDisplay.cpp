...
WaveformDisplay::WaveformDisplay(AudioEngine &engineToUse)
...
void WaveformDisplay::paint(juce::Graphics &g) {
  ...
  if (thumbnail.getTotalLength() <= 0.0) {
    ...
    g.drawFittedText("No Audio Recorded", getLocalBounds(),
  ...
  if (state == AudioEngine::TransportState::Playing) {
    ...
    // if we ware recording the playhead is always at the end of the wave where
    // we currentyl write data
  ...
  if (currentPosition > 0.0 && thumbnail.getTotalLength() > 0.0) {
    auto playheadPositionX =
        (currentPosition / thumbnail.getTotalLength()) * getWidth();

    g.setColour(juce::Colours::white);
    g.drawLine(static_cast<float>(playheadPositionX), 0.0f,
               static_cast<float>(playheadPositionX),
